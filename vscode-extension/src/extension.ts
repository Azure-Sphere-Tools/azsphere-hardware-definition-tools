...
export function activate(context: ExtensionContext) {
	...
	if (context.extensionMode == ExtensionMode.Development || context.extensionMode == ExtensionMode.Test ) {
		...
		serverModule = context.asAbsolutePath(
			path.join('embedded-language-server', 'node_modules', 'azsphere-hardware-definition-language-server', 'dist', 'server.js')
		);
	}
	
	console.log(`Extension running in ${ExtensionMode[context.extensionMode]} mode. Running language server from ${serverModule}`);
