...
export function appConflictDuplicateValue(conflictPinName: string, partnerComponentId: string, range: Range) {
  return{
    code: APP_DUPLICATE_VALUE_WARNING_CODE,
    message: `App manifest value of ${conflictPinName} is also declared in partner app ${partnerComponentId}.`,
    range: range,
    severity: DiagnosticSeverity.Warning,
    source: EXTENSION_SOURCE
  };
