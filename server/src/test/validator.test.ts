...
suite('validateNamesAndMappings', () => {
	...
	test('Validate Indirect Mapping', () => {
		...
		const hwDefFalseImported = new HardwareDefinition(asURI(hwDefFilePathFalseImported), undefined, [odmPin], [hwDefWithSourcePin]);
...
suite('validateApplicationManifest', () => {
    ...
    const hwDefFilePath = "my_app/hardwareDef.json";
		...
		assert.strictEqual(warningDiagnostics[0].message, "App manifest value of $SAMPLE_I2C1 is also declared in partner app FGHIJ.");
		...
		assert.strictEqual(warningDiagnostics[2].message, "App manifest value of ADC-CONTROLLER-0 is also declared in partner app FGHIJ.");
