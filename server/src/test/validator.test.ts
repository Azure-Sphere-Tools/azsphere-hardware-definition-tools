...
suite('validateNamesAndMappings', () => {
	...
	test('Sends warnings iff pins with duplicate mappings are in the same hardware definition', () => {
		...
		const mt3620 = getDummyImport({
			hardwareDefinition: new HardwareDefinition('mt3620.json', undefined, [mt3620_peripheral_0, mt3620_peripheral_1, mt3620_peripheral_2])
		});
		const avnet = getDummyImport({
			hardwareDefinition: new HardwareDefinition('avnet.json', undefined, [avnet_peripheral_0, avnet_peripheral_1], [mt3620])
		});
